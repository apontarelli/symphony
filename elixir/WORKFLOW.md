...
- If the target is `main`, preserve the existing mainline pull/push/review/land behavior.
- If the target is not `main`, pull from `origin/<target>`, create or update the PR with base `<target>`, and never merge or promote anything to `main` in v1.
- Before handoff and landing, enforce profile-specific gates and completion requirements present in the policy JSON, including `checks`, `completion_requirements`, `validation`, or `review` fields.
...
4. Check whether a PR already exists for the current branch and whether it is closed.
   ...
   - Create a fresh branch from `origin/<delivery.pr_target>` and restart execution flow as a new attempt.
...
5. Create a fresh branch from `origin/<delivery.pr_target>`.
...
- For closed/merged branch PRs, create a new branch from `origin/<delivery.pr_target>` and restart from reproduction/planning as if starting fresh.
