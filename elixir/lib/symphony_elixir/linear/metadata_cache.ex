defmodule SymphonyElixir.Linear.MetadataCache do
  ...
  def get(connection, opts) when is_map(connection) and is_list(opts) do
    ...
    timeout = Keyword.get(opts, :call_timeout, @default_call_timeout_ms)
  ...
  def handle_call({:get, connection, request_opts}, _from, state) do
    ...
    cond do
      ...
      should_schedule?(entry, now, state, force_refresh?) ->
        case schedule_job(state, identity, connection, request_opts, now) do
          {:ok, state} ->
  ...
  defp now(%State{now_fun: now_fun}), do: now_fun.()
  ...
  defp schedule_job(state, identity, connection, request_opts, now) do
    if map_size(state.jobs) >= state.max_concurrent_jobs do
      {:error, :capacity, state}
    else
      parent = self()
      job_ref = make_ref()
      fetch_opts = effective_fetch_opts(state, request_opts, identity)
        ...
        spawn_link(fn ->
          receive do
            {:metadata_cache_bind, ^job_ref} ->
              result = run_fetch(fetch_fun, connection, fetch_opts)
  ...
  defp effective_fetch_opts(state, request_opts, identity) do
    ...
    bound_env_fetcher = fn ^environment_name -> resolved_secret end

    Keyword.put(configured, :env_fetcher, bound_env_fetcher)
  end

  defp run_fetch(fetch_fun, connection, fetch_opts) do
    if fetch_fun, do: fetch_fun.(connection), else: Metadata.fetch(connection, fetch_opts)
  rescue
  ...
  defp settle_job(state, job_ref, result) do
    case Map.pop(state.jobs, job_ref) do
      ...
      {%{revision: revision, timer_ref: timer_ref}, jobs} ->
        Process.cancel_timer(timer_ref)
        state = %{state | jobs: jobs}
        now = now(state)

        entry = Map.fetch!(state.entries, revision)
        put_entry(state, revision, apply_fetch_result(entry, result, now, state))
    end
  end
