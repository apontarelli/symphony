defmodule SymphonyElixir.AgentRuntime.CodexAppServer do
  @moduledoc """
  Codex app-server adapter for the AgentRuntime seam.

  This module owns the native Codex JSON-RPC 2.0 stream, app-server launch
  details, dynamic tool calls, approvals, input handling, and conversion into
  Symphony runtime update events.
  """
  ...
  def capabilities(_adapter_config) do
    capabilities()
  end
  ...
  def run_turn(
    ...
    case start_turn(port, thread_id, prompt, issue, workspace, approval_policy, turn_sandbox_policy) do
      {:ok, turn_id} ->
        ...
        case await_turn_completion(port, on_event, tool_executor, auto_approve_requests, turn_context, timeout_ms) do
          {:ok, result} ->
      ...
      {:error, reason} ->
        ...
        emit_message(on_event, :startup_failed, %{reason: reason}, metadata)
        {:error, reason}
    end
  end
  ...
  defp send_initialize(port, timeout) do
    payload = %{
      "method" => "initialize",
      "id" => @initialize_id,
      "params" => %{
  ...
  defp start_thread(port, workspace, %{approval_policy: approval_policy, thread_sandbox: thread_sandbox}, timeout) do
    send_message(port, %{
      "method" => "thread/start",
      "id" => @thread_start_id,
      "params" => %{
  ...
  defp await_turn_completion(port, on_event, tool_executor, auto_approve_requests, turn_context, timeout_ms) do
    receive_loop(
      %{
        port: port,
        on_event: on_event,
  ...
  defp handle_incoming(context, data) do
    ...
    case Jason.decode(payload_string) do
      {:ok, %{"method" => "turn/completed"} = payload} ->
        emit_turn_event(context.on_event, :turn_completed, payload, payload_string, context.port, payload)
        {:ok, :turn_completed}
  ...
  defp emit_turn_event(on_event, event, payload, payload_string, port, payload_details) do
    emit_message(
      on_event,
      event,
      %{
  ...
  defp handle_turn_method(context, payload, payload_string, method) do
    ...
    case record_error_loop_candidate(
      ...
      {:error, reason, details} ->
        emit_message(context.on_event, :codex_error_loop, details, metadata)
        {:error, reason}
  ...
  defp maybe_handle_approval_request(
    ...
    emit_message(on_event, :tool_call, %{payload: payload, raw: payload_string}, metadata)
    ...
    emit_message(on_event, event, %{payload: payload, raw: payload_string}, metadata)
  ...
  defp emit_message(on_event, event, details, metadata) when is_function(on_event, 1) do
    payload = metadata |> Map.merge(details)
  ...
  defp normalized_event_type(event)
       when event in [
              :session_started,
              :turn_started,
              :message_delta,
  ...
  defp normalized_event_type(event)
       when event in [:turn_input_required, :approval_required],
       do: :blocked

  defp normalized_event_type(event)
       when event in [:turn_ended_with_error, :startup_failed, :codex_error_loop, :turn_cancelled],
       do: :turn_failed

  defp normalized_event_type(event)
       when event in [:tool_call_completed, :tool_call_failed, :unsupported_tool_call, :tool_input_auto_answered],
       do: :tool_result

  defp normalized_event_type(_event), do: :message_delta

  defp native_payload(%{payload: %{} = payload}), do: payload
  defp native_payload(_details), do: nil

  defp blocked_reason(:turn_input_required), do: :operator_input_requested
  defp blocked_reason(:approval_required), do: :approval_required
  defp blocked_reason(_event), do: nil
  ...
  defp default_on_event(_message), do: :ok
