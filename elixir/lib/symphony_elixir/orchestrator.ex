  ...
  defp maybe_dispatch(%State{} = state) do
    ...
    with :ok <- Config.validate!(),
      ...
      {:error, :missing_linear_api_token} ->
        Logger.error("Linear API token missing in selected workflow source")
        state

      {:error, :missing_linear_project_slug} ->
        Logger.error("Linear project slug missing in selected workflow source")
        state

      {:error, :missing_tracker_kind} ->
        Logger.error("Tracker kind missing in selected workflow source")

        state

      {:error, {:unsupported_tracker_kind, kind}} ->
        Logger.error("Unsupported tracker kind in selected workflow source: #{inspect(kind)}")

        state

      {:error, {:invalid_workflow_config, message}} ->
        Logger.error("Invalid workflow config: #{message}")
        state

      {:error, {:missing_workflow_file, path, reason}} ->
        Logger.error("Missing workflow source at #{path}: #{inspect(reason)}")
        state

      {:error, :workflow_front_matter_not_a_map} ->
        Logger.error("Failed to parse workflow source: workflow front matter must decode to a map")
        state

      {:error, {:workflow_parse_error, reason}} ->
        Logger.error("Failed to parse workflow source: #{inspect(reason)}")
        state
