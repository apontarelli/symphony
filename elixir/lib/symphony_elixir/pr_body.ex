defmodule SymphonyElixir.PrBody do
  @moduledoc false
  ...
  def validate_file(path) when is_binary(path) do
    with {:ok, template_path, template} <- read_template(),
         ...
         {:ok, headings} <- extract_template_headings(template, template_path) do
      lint(template, body, headings)
    end
  end

  @spec validate_body(String.t()) :: :ok | {:error, String.t()}
  ...
  def validate_body(_body), do: {:error, "PR body must be a string."}
